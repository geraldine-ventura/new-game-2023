...
pygame.display.set_caption("Shooter_Girl")
...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, bullet, grenades):
        ...
        for animation in animation_types:
            ...
            num_of_frames = len(
                os.listdir(f"img/caracters/{self.char_type}/{animation}")
            )
            for i in range(num_of_frames):
                # img/caracters/player/Dead/Dead (2).png
                if i > 0 and i <= num_of_frames:
                    img = pygame.image.load(
                        f"img/caracters/{self.char_type}/{animation}/{animation} ({i}).png"
                    ).convert_alpha()
            ...
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile >= 0 and tile <= 8:
                        ...
                        player = Soldier(
                            "player", x * TILE_SIZE, y * TILE_SIZE, 0.09, 5, 20, 5
                        )
                        health_bar = HealthBar(10, 10, player.health, player.health)
                    elif tile == 16:  # create enemies
                        enemy = Soldier(
                            "enemy", x * TILE_SIZE, y * TILE_SIZE, 0.04, 2, 20, 0
                        )
                        enemy_group.add(enemy)
                    elif tile == 17:  # create bullet box
...
while run:
    ...
    if start_game == False:
        ...
        # show bullet
        draw_text("bullet: ", font, WHITE, 10, 35)
